<x-app-layout>
    ...
    <div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
        ...
        <div class="p-4 m-2 bg-white rounded-lg shadow-sm border border-gray-100">
            ...
            <form action="{{ route('blog.all') }}" method="post" class="space-y-4">
                @csrf
                <div class="flex">
                    <div class="flex items-center me-4">
                        ...
                        <div class="flex items-center mb-2">
                            <input id="cat-{{ $cat->id }}"
                                ...
                                @checked(is_array(old('category')) && in_array($cat->id, old('category')))>
                ...
                <div class="flex">
                    <button type="submit" class="flex justify-center py-2 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors">
                        Filter
                    </button>
                </div>
    ...
    $(document).ready(function() {
        $('.category-checkbox').on('change', function() {
            ...
            $('.category-checkbox:checked').each(function() {
                ...
                selected.push($(this).next('span').text());
            });

            if (selected.length > 0) {
                $('#selected-list').text(selected.join(', '));
                $('#selected-items').removeClass('hidden');
            } else {
                $('#selected-items').addClass('hidden');
            }
        });
    });
</script>
