<x-app-layout>
    ...
    <form action="{{ route('blog.updateBlog', $blog->id) }}" method="POST" class="max-w-xl mx-auto p-4 space-y-4 border rounded shadow" id="blogFormId" enctype="multipart/form-data">
        @csrf

        <!-- title -->
        <div>
            <x-input-label for="title" :value="__('Title')" :required="true"/>
            <x-text-input id="title" class="block mt-1 w-full" type="text" name="title" :value="old('title' , $blog->title)" placeholder="Post Title" />
        ...
        <div>
            ...
            <select class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 w-full mt-1 p-2 border rounded" name="category">
                ...
                <option value="{{ $cat->name }}"
                    {{ $blog->category['name'] == $cat->name ? 'selected' : '' }}>
                    {{ $cat->name }}
                </option>
                @endforeach
                @endif
        ...
        <div>
            ...
            <x-textarea-input name="content" id="content" value="{{ $blog->content }}">{{ old('content') }}</x-textarea-input>
        ...
        <a href="{{ route('user.dashboard') }}"
            class="ml-2 border border-gray-300 text-gray-700 px-4 py-2 rounded hover:bg-gray-50 transition">
            Back
        </a>
